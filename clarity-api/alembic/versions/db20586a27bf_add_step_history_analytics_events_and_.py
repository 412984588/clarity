...
def upgrade() -> None:
    ...
    sa.ForeignKeyConstraint(['session_id'], ['solve_sessions.id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id')
    )
    ...
    op.add_column('solve_sessions', sa.Column('locale', sa.String(length=10), nullable=True))
    op.add_column('solve_sessions', sa.Column('first_step_action', sa.Text(), nullable=True))
    op.add_column('solve_sessions', sa.Column('reminder_time', sa.DateTime(), nullable=True))
    ...
    op.create_unique_constraint('uq_usage_user_period', 'usage', ['user_id', 'period_start'])
...
def downgrade() -> None:
    ...
    op.create_index(op.f('ix_solve_sessions_user_status'), 'solve_sessions', ['user_id', 'status'], unique=False)
    op.create_index(op.f('ix_solve_sessions_user_id'), 'solve_sessions', ['user_id'], unique=False)
