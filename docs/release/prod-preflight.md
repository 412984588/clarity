...
| `APPLE_CLIENT_ID` | `com.clarity.app` | [Apple Developer Portal](https://developer.apple.com/account/resources/identifiers) |
...
| `STRIPE_SUCCESS_URL` | `https://api.clarity.app/subscriptions/success?session_id={CHECKOUT_SESSION_ID}` | 固定格式 |
| `STRIPE_CANCEL_URL` | `https://api.clarity.app/subscriptions/cancel` | 固定格式 |
...
# Project Settings → Domains → Add → api.clarity.app

# 5.2 验证 SSL
curl -I https://api.clarity.app
# 预期: HTTP/2 200, 有效证书
```
...
**结论**: 本地 smoke 无法执行，需在实际部署后运行 `./scripts/deploy_prod_smoke.sh https://api.clarity.app`
