...
def test_validate_session_returns_active_session():
    ...
    # Act
    result = learn_router._validate_session(session)
...
def test_build_context_prompt_returns_current_message_without_history():
    ...
    # Act
    result = learn_router._build_context_prompt(history, message)
...
def test_generate_review_schedule_uses_expected_offsets(monkeypatch):
    ...
    monkeypatch.setattr(learn_router, "utc_now", lambda: fixed_now)

    # Act
    result = learn_router._generate_review_schedule()
