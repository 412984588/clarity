...
class BetaLoginRequest(BaseModel):
    device_fingerprint: Optional[str] = Field(
        ...
        examples=["beta-5b7c9a1d"],
    )
    device_name: Optional[str] = Field(
        default=None,
        description="设备名称（未提供时使用默认值）",
        examples=["Beta Device"],
    )
...
class OAuthRequest(BaseModel):
    id_token: str = Field(
        ...
        examples=["eyJhbGciOiJSUzI1NiIsImtpZCI6IjE2..."],
    )
    device_fingerprint: str = Field(
        ...,
        description="设备指纹",
...
class ErrorResponse(BaseModel):
    ...
    detail: Optional[str] = Field(
        default=None,
        description="错误详情说明",
        examples=["Token is missing or invalid"],
    )
...
class CsrfTokenResponse(BaseModel):
    csrf_token: str = Field(
        ...
        examples=["p2gq5q1oY2B2b0vZ5d9dJq8w5uQ0j3nX1y9uK1dG7a0"],
    )
