 ...
 */
export const getRecentMessages = async (sessionId: string, limit: number = 50): Promise<Message[]> => {
  const database = getDb();
  const result = await database.getAllAsync<Message>(
 ...
 * 插入一个选项
 */
export const insertOption = async (option: Omit<Option, 'is_selected' | 'created_at'>): Promise<void> => {
  const database = getDb();
  await database.runAsync(
 ...
 * 批量插入选项
 */
export const insertOptions = async (options: Omit<Option, 'is_selected' | 'created_at'>[]): Promise<void> => {
  const database = getDb();
